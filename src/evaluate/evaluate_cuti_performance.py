...
def main():
    ...
    # Create the logger 
    log_dir = os.path.join(os.path.dirname(__file__), '../..', f'logs/cuti/probe-{args.lr}|/{seed}')
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    
    log_path = os.path.join(log_dir, f'cuti_{source_domain}_to_{target_domain}.log')
    ...
    # finetune_model(model_copy, source_trainloader, source_testloader, lr=args.lr)
