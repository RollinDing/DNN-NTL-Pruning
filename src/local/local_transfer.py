...
def local_transfer(model, source_trainloader, source_testloader):
...
def main():
    ...
    log_path = os.path.join(log_dir, f'{args.source}-{args.target}.log')
    ...
    logging.info(f'Local transfer learning from {source_domain} to {target_domain} dataset')
    ...
    logging.info(f'Data ratio {finetune_ratio}, Data volume {data_volume},  Local transfer from {source_domain} to {target_domain} dataset, the best accuracy is {best_acc}')
