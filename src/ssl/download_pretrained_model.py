# Download use the pretrained model
#!/usr/bin/env python
# coding: utf-8
...
class LoadedResNet(nn.Module):
    def __init__(self, model_name):
        ...
        if model_name == 'supervised':
            ...
            path = os.path.join('base_models/ssl_models', f'{self.model_name}.pth')
            print(path)
...
def download_pretrained_models(pretrained_models_path='base_models/ssl_models'):
...
if __name__ == '__main__':
    os.makedirs("./base_models/ssl_models", exist_ok=True)
    ...
    if os.path.isfile('base_models/ssl_models/byol.pkl'):
        # BYOL is provided in a pickle format, so we need to handle that first
        convert_byol('base_models/ssl_models/byol.pkl', 'base_models/ssl_models/byol.pth')
        os.remove('base_models/ssl_models/byol.pkl')
    supervised_model = models.resnet50(pretrained=True)
    # save the model in format of pretrained resnet50 model in pth
    torch.save(supervised_model.state_dict(), os.path.join('base_models/ssl_models', 'supervised.pth'))
    ...
    for model_name in MODELS:
        ...
        torch.save(state_dict, os.path.join('base_models/ssl_models', f'{model_name}.pth'))